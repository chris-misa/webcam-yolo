...
* `webcam_yolo_osc.py` is a python script that loads a given (trained) yolo model, reads frames from a webcam, detects the highest-confidence object using the model, and sends an OSC message to /yolo with the x and y coordinates of the center of the detected object. Lots of parameters are set as variable at the top of the file.
